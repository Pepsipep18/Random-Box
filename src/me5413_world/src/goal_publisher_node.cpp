...
GoalPublisherNode::GoalPublisherNode() : tf2_listener_(tf2_buffer_)
{
  ...
  // Gai
  // this->sub_move_base_status_ = nh_.subscribe("/move_base/status", 10, &GoalPublisherNode::moveLoop, this);
...
{   
  ...
  // x: 8.0 to 16.0, y: -6.25 to 1.0, yaw: -3.14 to 3.14
    ...
    // std::mt19937 gen(rd());  
    // std::uniform_real_distribution<double> x_dist(8.0, 16.0);  
    // std::uniform_real_distribution<double> y_dist(-6.25, 0.0);  
    // std::uniform_real_distribution<double> yaw_dist(-3.14, 3.14); 
    // double x = std::round(x_dist(gen) * 10) / 10.0;
    // double y = std::round(y_dist(gen) * 10) / 10.0;
    // double yaw = std::round(yaw_dist(gen) * 10) / 10.0;
...
// void GoalPublisherNode::moveLoop(const actionlib_msgs::GoalStatusArray& status)
// {
...
//           geometry_msgs::PoseStamped nextLoopPoint = this->SetTargetPoint();
...
//           }
//           geometry_msgs::PoseStamped P_map_goal;
//           tf2::doTransform(nextLoopPoint, P_map_goal, transform_map_world);
//           P_map_goal.header.stamp = ros::Time::now();
//           P_map_goal.header.frame_id = map_frame_;
